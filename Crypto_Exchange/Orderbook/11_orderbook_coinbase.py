#!/usr/bin/env python3
"""Coinbase Order Book Feed for CLOB Aggregator"""
...
class CoinbaseOrderBook:
    """Coinbase WebSocket order book feed"""
    ...
    def start(self):
        ...
        self.running = True
        Thread(target=self._run, daemon=True, name="coinbase").start()
                ...
                while self.running:
                    ...
                    if data.get("type") == "snapshot":
                        for price_str, size_str in data["bids"]:
                            self.order_book["bids"][float(price_str)] = float(size_str)
                        for price_str, size_str in data["asks"]:
                            self.order_book["asks"][float(price_str)] = float(size_str)
                    ...
                    if self.order_book["bids"] and self.order_book["asks"]:
                        ...
                        if bids and asks:
                            self.aggregator.update_exchange('coinbase', bids, asks)
