#!/usr/bin/env python3
"""HTX (Huobi) Order Book Feed for CLOB Aggregator"""
...
class HTXOrderBook:
    """HTX (Huobi) WebSocket order book feed"""
    ...
    def start(self):
        ...
        self.running = True
        Thread(target=self._run, daemon=True, name="htx").start()
    ...
    def _run(self):
        ...
        while self.running:
            try:
                ...
                subscribe_msg = {
                    "sub": f"market.{self.symbol}.depth.step0",
                    "id": "id1"
                }
                ws.send(json.dumps(subscribe_msg))
                print(f"✓ HTX connected")
                ...
                while self.running:
                    ...
                    if bids and asks:
                        self.aggregator.update_exchange('htx', bids, asks)
