    ...
    auto parse_levels = [](const std::string& text, size_t start) -> std::vector<Level> {
        ...
        while (pos < text.size() && text[pos] != ']') {
            if (text[pos] == '[') {
                ...
                std::string triple = text.substr(pos + 1, end - pos - 1);
                
                // Remove quotes: ["69500.5","1.234","5"] -> 69500.5,1.234,5
                std::string cleaned;
                for (char c : triple) {
                    if (c != '"') cleaned += c;
                }
                
                // Parse: price,volume,count (we only need first 2)
                size_t comma1 = cleaned.find(',');
                if (comma1 != std::string::npos) {
                    size_t comma2 = cleaned.find(',', comma1 + 1);
                    if (comma2 != std::string::npos) {
                        try {
                            double price = std::stod(cleaned.substr(0, comma1));
                            double volume = std::stod(cleaned.substr(comma1 + 1, comma2 - comma1 - 1));
                            if (volume > 0.0) {
    ...
    while (running_) {
        try {
            ...
            ws.set_option(websocket::stream_base::decorator([](websocket::request_type& req) {
                req.set(http::field::user_agent, "Cryptocom-OrderBook-C++20");
            }));
            ...
            std::string subscribe = R"({"id":1,"method":"subscribe","params":{"channels":["book.BTC_USDT.10"]},"nonce":)" 
                + std::to_string(nonce) + "}";
            ws.write(net::buffer(subscribe));
            
            std::cout << "✓ Crypto.com connected\n";
            ...
            while (running_) {
                ...
                if (message.find("\"public/heartbeat\"") != std::string::npos) {
                    size_t id_pos = message.find("\"id\"");
                    if (id_pos != std::string::npos) {
                        size_t colon = message.find(':', id_pos);
                        size_t end = message.find_first_of(",}", colon);
                        if (colon != std::string::npos && end != std::string::npos) {
                            std::string id_val = message.substr(colon + 1, end - colon - 1);
                            std::string response = R"({"id":)" + id_val + R"(,"method":"public/respond-heartbeat"})";
                ...
                if (message.find("\"result\"") != std::string::npos && message.find("\"data\"") != std::string::npos) {
            ...
            if (running_) {
                std::cerr << "Crypto.com error: " << e.what() << ", reconnecting...\n";
