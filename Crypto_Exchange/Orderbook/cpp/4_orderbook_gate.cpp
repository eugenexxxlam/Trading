...
GateOrderBook::GateOrderBook(const std::string& symbol, int depth)
    : m_symbol(symbol), m_depth(depth) {
    m_bids.reserve(depth);
    m_asks.reserve(depth);
}

GateOrderBook::~GateOrderBook() {
    stop();
}

void GateOrderBook::start() {
    if (m_running.exchange(true)) return;
    m_thread = std::thread(&GateOrderBook::run_loop, this);
}

void GateOrderBook::stop() {
    m_running = false;
    if (m_thread.joinable()) m_thread.join();
}

bool GateOrderBook::process_message(const std::string& msg) {
    // Gate.io sends full snapshots: {"event":"update","result":{"asks":[["p","v"]],"bids":[["p","v"]]}}
    try {
        if (msg.find("\"event\":\"update\"") == std::string::npos || 
            msg.find("\"result\"") == std::string::npos) return false;
...
void GateOrderBook::run_loop() {
    while (m_running) {
        try {
            ...
            SSL_set_tlsext_host_name(ws.next_layer().native_handle(), "api.gateio.ws");
            ws.next_layer().handshake(ssl::stream_base::client);

            ws.handshake("api.gateio.ws", "/ws/v4/");
            
            // Subscribe: {"time":timestamp,"channel":"spot.order_book","event":"subscribe","payload":["BTC_USDT","20","100ms"]}
            auto timestamp = std::chrono::duration_cast<std::chrono::seconds>(
                std::chrono::system_clock::now().time_since_epoch()).count();
            std::string sub = "{\"time\":" + std::to_string(timestamp) + 
                            ",\"channel\":\"spot.order_book\",\"event\":\"subscribe\",\"payload\":[\"" + 
                            m_symbol + "\",\"20\",\"100ms\"]}";
            ...
            if (m_running) {
                std::cerr << "Gate.io error: " << e.what() << ", reconnecting...\n";
...
void display_orderbook(const GateOrderBook& ob, int levels = 16) {
    ...
    std::cout << std::setw(35) << std::right << "Exchange: Gate.io" << std::setw(35) << std::left << "" << "\n";
...
int main() {
    ...
    std::cout << "Starting Gate.io OrderBook Feed (C++20)...\n";
    std::cout << "Connecting to Gate.io WebSocket...\n\n";
    
    GateOrderBook ob("BTC_USDT", 20);
    ob.start();
