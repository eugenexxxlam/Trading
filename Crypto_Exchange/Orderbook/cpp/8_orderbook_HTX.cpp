...
void OrderBook::process_message(const std::string& msg) {
    ...
    if (msg.find("\"subbed\"") != std::string::npos || 
        (msg.find("\"id\"") != std::string::npos && msg.find("\"tick\"") == std::string::npos)) {
        return;
    }
        ...
        while (pos < text.size() && text[pos] != ']') {
            if (text[pos] == '[') {
                ...
                if (comma != std::string::npos) {
                    try {
                        double price = std::stod(pair.substr(0, comma));
                        double volume = std::stod(pair.substr(comma + 1));
                        if (volume > 0.0) {
    ...
    auto tick_pos = msg.find("\"tick\"");
    if (tick_pos == std::string::npos) return;
    
    auto bids_pos = msg.find("\"bids\"", tick_pos);
    auto asks_pos = msg.find("\"asks\"", tick_pos);
...
void OrderBook::run_loop() {
    while (running_) {
        try {
            ...
            auto const results = resolver.resolve("api.huobi.pro", "443");
            auto ep = net::connect(get_lowest_layer(ws), results);
            
            if (!SSL_set_tlsext_host_name(ws.next_layer().native_handle(), "api.huobi.pro")) {
            ...
            ws.set_option(websocket::stream_base::decorator([](websocket::request_type& req) {
                req.set(http::field::user_agent, "HTX-OrderBook-C++20");
            }));
            
            ws.handshake("api.huobi.pro", "/ws");
            
            std::string subscribe = R"({"sub":"market.btcusdt.depth.step0","id":"id1"})";
            ws.write(net::buffer(subscribe));
            
            std::cout << "✓ HTX connected\n";
            ...
            while (running_) {
                ...
                if (decompressed.find("\"tick\"") != std::string::npos) {
            ...
            if (running_) {
                std::cerr << "HTX error: " << e.what() << ", reconnecting...\n";
