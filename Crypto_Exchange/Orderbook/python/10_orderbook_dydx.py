#!/usr/bin/env python3
"""dYdX DEX Order Book Feed for CLOB Aggregator"""
...
class DydxOrderBook:
    """dYdX DEX WebSocket order book feed"""
    
    def __init__(self, aggregator, symbol="BTC-USD", depth=20):
        self.aggregator = aggregator
        self.symbol = symbol
        self.depth = depth
        self.running = False
        self.url = "wss://indexer.dydx.trade/v4/ws"
        self.order_book = {"bids": {}, "asks": {}}
    
    def start(self):
        """Start WebSocket feed in background thread"""
        self.running = True
        Thread(target=self._run, daemon=True, name="dydx").start()
        ...
        while self.running:
            try:
                ...
                while self.running:
                    ...
                    if data.get("type") != "channel_data" or "contents" not in data:
                        continue
                    
                    contents = data["contents"]
                    
                    if "bids" in contents:
                        for price_str, size_str in contents["bids"]:
                    ...
                    if "asks" in contents:
                        for price_str, size_str in contents["asks"]:
                            price, size = float(price_str), float(size_str)
                            if size == 0:
                    ...
                    if self.order_book["bids"] and self.order_book["asks"]:
                        ...
                        if bids and asks:
                            self.aggregator.update_exchange('dydx', bids, asks)
                    
            except Exception as e:
                if self.running:
                    print(f"dYdX error: {e}, reconnecting...")
                    time.sleep(2)
