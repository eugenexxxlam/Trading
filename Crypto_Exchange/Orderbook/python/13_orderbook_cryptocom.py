#!/usr/bin/env python3
"""Crypto.com Order Book Feed for CLOB Aggregator"""
...
class CryptocomOrderBook:
    """Crypto.com WebSocket order book feed"""
    
    def __init__(self, aggregator, symbol="BTC_USDT", depth=20):
        self.aggregator = aggregator
        self.symbol = symbol
        self.depth = depth
        self.running = False
        self.url = "wss://stream.crypto.com/v2/market"
    
    def start(self):
        """Start WebSocket feed in background thread"""
        self.running = True
        Thread(target=self._run, daemon=True, name="cryptocom").start()
        ...
        while self.running:
            try:
                ...
                subscribe_msg = {
                    "id": 1,
                    "method": "subscribe",
                    "params": {"channels": [f"book.{self.symbol}.10"]},
                    "nonce": int(time.time() * 1000)
                }
                ws.send(json.dumps(subscribe_msg))
                print(f"✓ Crypto.com connected")
                
                while self.running:
                    data = json.loads(ws.recv())
                    
                    if data.get("method") == "public/heartbeat":
                        ws.send(json.dumps({"id": data["id"], "method": "public/respond-heartbeat"}))
                        continue
                    ...
                    bids = sorted(((float(p), float(v)) for p, v, _ in book_data["bids"]), 
                                  key=lambda x: x[0], reverse=True)
                    asks = sorted(((float(p), float(v)) for p, v, _ in book_data["asks"]), 
                                  key=lambda x: x[0])
                    ...
                    if bids and asks:
                        self.aggregator.update_exchange('cryptocom', bids, asks)
    ...
    try:
        while True:
            ...
            if hasattr(agg, 'bids') and agg.bids and hasattr(agg, 'asks') and agg.asks:
                ...
                print()
                print(f"{'=' * 70}")
                print(f"{'Exchange: Crypto.com':^70}")
