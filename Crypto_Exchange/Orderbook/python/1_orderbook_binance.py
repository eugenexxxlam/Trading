#!/usr/bin/env python3
"""Binance Order Book Feed for CLOB Aggregator"""
...
class BinanceOrderBook:
    ...
    def __init__(self, aggregator, symbol="BTCUSDT", depth=20):
        ...
        self.depth = depth
        self.running = False
        self.url = f"wss://stream.binance.com:9443/ws/{symbol.lower()}@depth{depth}@100ms"
    
    def start(self):
        """Start WebSocket feed in background thread"""
        self.running = True
        Thread(target=self._run, daemon=True, name="binance").start()
        ...
        while self.running:
            try:
                ...
                while self.running:
                    ...
                    if bids and asks:
                        self.aggregator.update_exchange('binance', bids, asks)
