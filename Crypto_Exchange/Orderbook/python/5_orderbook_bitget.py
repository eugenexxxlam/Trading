#!/usr/bin/env python3
"""Bitget Order Book Feed for CLOB Aggregator"""
...
class BitgetOrderBook:
    """Bitget WebSocket order book feed"""
    
    def __init__(self, aggregator, symbol="BTCUSDT", depth=20):
    ...
    def start(self):
        ...
        self.running = True
        Thread(target=self._run, daemon=True, name="bitget").start()
    ...
    def _run(self):
        ...
        while self.running:
            try:
                ...
                subscribe_msg = {
                    "op": "subscribe",
                    "args": [{"instType": "SPOT", "channel": "books", "instId": self.symbol}]
                }
                ws.send(json.dumps(subscribe_msg))
                print(f"✓ Bitget connected")
                ...
                while self.running:
                    ...
                    if bids and asks:
                        self.aggregator.update_exchange('bitget', bids, asks)
