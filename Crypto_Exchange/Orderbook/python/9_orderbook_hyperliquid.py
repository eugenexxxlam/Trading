#!/usr/bin/env python3
"""Hyperliquid DEX Order Book Feed for CLOB Aggregator"""
...
class HyperliquidOrderBook:
    """Hyperliquid DEX WebSocket order book feed"""
    
    def __init__(self, aggregator, symbol="BTC", depth=20):
    ...
    def start(self):
        ...
        self.running = True
        Thread(target=self._run, daemon=True, name="hyperliquid").start()
        ...
        while self.running:
            try:
                ...
                while self.running:
                    ...
                    bids = sorted(((float(level["px"]), float(level["sz"])) for level in levels[0]),
                                  key=lambda x: x[0], reverse=True)
                    asks = sorted(((float(level["px"]), float(level["sz"])) for level in levels[1]),
                                  key=lambda x: x[0])
                    ...
                    if bids and asks:
                        self.aggregator.update_exchange('hyperliquid', bids, asks)
                    
            except Exception as e:
                if self.running:
                    print(f"Hyperliquid error: {e}, reconnecting...")
                    time.sleep(2)
    ...
    try:
        while True:
            ...
            if hasattr(agg, 'bids') and agg.bids and hasattr(agg, 'asks') and agg.asks:
                ...
                print()
                print(f"{'=' * 70}")
                print(f"{'Exchange: Hyperliquid DEX':^70}")
