//! HTX (Huobi) BTCUSDT orderbook - WebSocket live display at 20 FPS
...
fn parse_depth(json: &str) -> Option<Book> {
    let resp: HTXResponse = serde_json::from_str(json).ok()?;
    
    let tick = resp.tick?;
    
    let parse = |levels: &[Vec<f64>]| -> Vec<(f64, f64)> {
        levels.iter()
            .filter_map(|v| {
                if v.len() >= 2 && v[1] > 0.0 {
                    Some((v[0], v[1]))
    ...
    let (mut bids, mut asks) = (parse(&tick.bids), parse(&tick.asks));
...
fn render_orderbook(bids: &[(f64, f64)], asks: &[(f64, f64)], ups: f64, total: u64) -> String {
    ...
    let mut out = format!(
        "{CLEAR}{sep}\n{:^70}\n{:^70}\n{:^70}\n{:^70}\n{YELLOW}{:^70}{RESET}\n{sep}\n\n",
        "HTX (Huobi) BTCUSDT Orderbook (Rust)",
        ...
        move || {
            ...
            while r.load(Ordering::SeqCst) {
                if let Ok((mut ws, _)) = connect(URL) {
                    ...
                    let sub = r#"{"sub":"market.btcusdt.depth.step0","id":"id1"}"#;
                    ...
                    while r.load(Ordering::SeqCst) {
                        match ws.read() {
                            Ok(Message::Binary(data)) => {
                                ...
                                if let Ok(decompressed) = decompress_gzip(&data) {
                                    ...
                                    if let Ok(json) = serde_json::from_str::<Value>(&decompressed) {
                                        if let Some(ping) = json.get("ping").and_then(|v| v.as_u64()) {
                                            let pong = format!(r#"{{"pong":{}}}"#, ping);
                            ...
                            Ok(Message::Text(t)) => {
                                // Handle ping/pong in text format
                ...
                if r.load(Ordering::SeqCst) {
                    println!("HTX error, reconnecting...");
