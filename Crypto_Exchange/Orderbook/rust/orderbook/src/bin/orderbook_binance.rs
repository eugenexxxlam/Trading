//! Binance BTCUSDT orderbook - WebSocket live display at 20 FPS
...
fn parse_depth(json: &str) -> Option<Book> {
    let msg: DepthMessage = serde_json::from_str(json).ok()?;
    
    let parse = |levels: &[(String, String)]| -> Vec<(f64, f64)> {
        levels.iter()
            .filter_map(|(p, q)| Some((p.parse().ok()?, q.parse().ok()?)))
            .collect()
    };
    
    let (mut bids, mut asks) = (parse(&msg.bids), parse(&msg.asks));
...
fn render_orderbook(bids: &[(f64, f64)], asks: &[(f64, f64)], ups: f64, total: u64) -> String {
    ...
    let mut out = format!(
        "{CLEAR}{sep}\n{:^70}\n{:^70}\n{:^70}\n{:^70}\n{YELLOW}{:^70}{RESET}\n{sep}\n\n",
        "Binance BTCUSDT Orderbook (Rust)",
...
fn main() {
    ...
    thread::spawn({
        ...
        move || {
            const URL: &str = "wss://stream.binance.com:9443/ws/btcusdt@depth20@100ms";
