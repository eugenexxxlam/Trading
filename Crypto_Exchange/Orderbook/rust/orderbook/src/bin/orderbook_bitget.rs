//! Bitget BTCUSDT orderbook - WebSocket live display at 20 FPS
//! Note: Bitget sends gzip-compressed messages
...
fn parse_depth(json: &str) -> Option<Book> {
    let resp: BitgetResponse = serde_json::from_str(json).ok()?;
    ...
    let data = resp.data?;
    let book_data = data.into_iter().next()?;
...
fn main() {
    ...
    thread::spawn({
        ...
        move || {
            const URL: &str = "wss://ws.bitget.com/v2/ws/public";
            while r.load(Ordering::SeqCst) {
                if let Ok((mut ws, _)) = connect(URL) {
                    // Subscribe to order book
                    let sub = r#"{"op":"subscribe","args":[{"instType":"SPOT","channel":"books","instId":"BTCUSDT"}]}"#;
                    ...
                    while r.load(Ordering::SeqCst) {
                        match ws.read() {
                            Ok(Message::Text(t)) => {
                                // Handle ping/pong
                                if t.contains(r#""op":"ping""#) {
                                    let _ = ws.send(Message::Text(r#"{"op":"pong"}"#.to_string()));
                                    continue;
                                }
                            ...
                            Ok(Message::Binary(data)) => {
                                ...
                                if let Some(decompressed) = decompress_gzip(&data) {
                                    // Handle ping/pong
                                    if decompressed.contains(r#""op":"ping""#) {
                                        let _ = ws.send(Message::Text(r#"{"op":"pong"}"#.to_string()));
                                        continue;
                                    }
                                    
                                    if let Some(bk) = parse_depth(&decompressed) {
                                        *b.lock().unwrap() = bk;
                                        c.fetch_add(1, Ordering::Relaxed);
                                    }
                                }
                ...
                if r.load(Ordering::SeqCst) {
                    println!("Bitget error, reconnecting...");
