//! Bybit BTCUSDT orderbook - WebSocket live display at 20 FPS
...
fn parse_depth(json: &str) -> Option<Book> {
    let resp: BybitResponse = serde_json::from_str(json).ok()?;
    
    // Check if it's a snapshot or delta message
        ...
        move || {
            ...
            while r.load(Ordering::SeqCst) {
                if let Ok((mut ws, _)) = connect(URL) {
                    ...
                    let sub = r#"{"op":"subscribe","args":["orderbook.50.BTCUSDT"]}"#;
                ...
                if r.load(Ordering::SeqCst) {
                    println!("Bybit error, reconnecting...");
