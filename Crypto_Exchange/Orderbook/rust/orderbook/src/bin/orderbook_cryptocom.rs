//! Crypto.com BTC_USDT orderbook - WebSocket live display at 20 FPS
...
fn parse_depth(json: &str) -> Option<Book> {
    let msg: CryptocomResponse = serde_json::from_str(json).ok()?;
    
    // Skip heartbeat messages
    if msg.method.as_deref() == Some("public/heartbeat") {
        return None;
    }
    ...
    let bids_raw = book_data.bids.as_ref()?;
    let asks_raw = book_data.asks.as_ref()?;
...
fn render_orderbook(bids: &[(f64, f64)], asks: &[(f64, f64)], ups: f64, total: u64) -> String {
    ...
    let mut out = format!(
        "{CLEAR}{sep}\n{:^70}\n{:^70}\n{:^70}\n{:^70}\n{YELLOW}{:^70}{RESET}\n{sep}\n\n",
        "Crypto.com BTC_USDT Orderbook (Rust)",
...
fn main() {
    ...
    thread::spawn({
        ...
        move || {
            const URL: &str = "wss://stream.crypto.com/v2/market";
            while r.load(Ordering::SeqCst) {
                if let Ok((mut ws, _)) = connect(URL) {
                    ...
                    while r.load(Ordering::SeqCst) {
                        match ws.read() {
                            Ok(Message::Text(t)) => {
                                // Handle heartbeat
                                if t.contains("\"public/heartbeat\"") {
                                    if let Ok(msg) = serde_json::from_str::<CryptocomResponse>(&t) {
                                ...
                                // Only process messages with result and data
                                if t.contains("\"result\"") && t.contains("\"data\"") {
                ...
                if r.load(Ordering::SeqCst) {
                    eprintln!("Crypto.com error, reconnecting...");
