//! Gate.io BTCUSDT orderbook - WebSocket live display at 20 FPS
...
fn parse_depth(json: &str) -> Option<Book> {
    let resp: GateResponse = serde_json::from_str(json).ok()?;
...
fn render_orderbook(bids: &[(f64, f64)], asks: &[(f64, f64)], ups: f64, total: u64) -> String {
    ...
    let mut out = format!(
        "{CLEAR}{sep}\n{:^70}\n{:^70}\n{:^70}\n{:^70}\n{YELLOW}{:^70}{RESET}\n{sep}\n\n",
        "Gate.io BTC_USDT Orderbook (Rust)",
        ...
        move || {
            ...
            while r.load(Ordering::SeqCst) {
                if let Ok((mut ws, _)) = connect(URL) {
                    ...
                    let sub = format!(
                        r#"{{"time":{},"channel":"spot.order_book","event":"subscribe","payload":["BTC_USDT","20","100ms"]}}"#,
                        timestamp
                    );
                    
                    if ws.send(Message::Text(sub)).is_ok() {
                        println!("✓ Gate.io connected");
                    }
                ...
                if r.load(Ordering::SeqCst) {
                    println!("Gate.io error, reconnecting...");
