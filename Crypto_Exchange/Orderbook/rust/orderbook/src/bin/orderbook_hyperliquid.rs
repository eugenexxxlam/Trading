//! Hyperliquid DEX BTCUSD orderbook - WebSocket live display at 20 FPS
...
#[derive(Deserialize)]
struct BookData {
    levels: Option<Vec<Vec<LevelEntry>>>,
}
...
fn parse_depth(json: &str) -> Option<Book> {
    let resp: HyperliquidResponse = serde_json::from_str(json).ok()?;
    
    // Check if it's a l2Book channel message
    if resp.channel.as_deref() != Some("l2Book") {
        return None;
    }
    
    let book_data = resp.data?;
    let levels = book_data.levels?;
    ...
    let parse = |level_array: &[LevelEntry]| -> Vec<(f64, f64)> {
        level_array.iter()
            .filter_map(|level| {
                let price = level.px.parse().ok()?;
                let volume = level.sz.parse().ok()?;
                Some((price, volume))
            })
            .collect()
    };
    
    let (mut bids, mut asks) = (parse(&levels[0]), parse(&levels[1]));
...
fn render_orderbook(bids: &[(f64, f64)], asks: &[(f64, f64)], ups: f64, total: u64) -> String {
    ...
    let mut out = format!(
        "{CLEAR}{sep}\n{:^70}\n{:^70}\n{:^70}\n{:^70}\n{YELLOW}{:^70}{RESET}\n{sep}\n\n",
        "Hyperliquid DEX BTC-USD Orderbook (Rust)",
        ...
        move || {
            ...
            while r.load(Ordering::SeqCst) {
                if let Ok((mut ws, _)) = connect(URL) {
                    // Subscribe to l2Book
                    let sub = r#"{"method":"subscribe","subscription":{"type":"l2Book","coin":"BTC"}}"#;
                ...
                if r.load(Ordering::SeqCst) {
                    println!("Hyperliquid error, reconnecting...");
