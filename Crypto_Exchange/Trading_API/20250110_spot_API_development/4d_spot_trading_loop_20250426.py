...
RANDOM_TRADE_COUNT = 60000000
TOTAL_VOLUME = 100698  # Total amount to buy/sell
ORDER_COUNT = 3    # Number of market orders to be placed
WAIT_TIME = 5     # <-- Constant wait time in seconds
LIQUIDATION_TIME = 60
...
def open_position_in_spot(api_key, secret_key, symbol_name):
    ...
    Ensures that the volume is truncated based on the symbol's precision.
    """
    # Fetch the precision for the symbol
    symbol_precision = get_symbol_precision(symbol_name, precision_data)

    if symbol_precision['quantityPrecision'] == 'N/A':
        logging.error(f"Quantity precision for {symbol_name} not found. Skipping order.")
        return

    per_order_volume = TOTAL_VOLUME / ORDER_COUNT

    # Truncate the per_order_volume based on the symbol's precision
    per_order_volume = truncate(per_order_volume, symbol_precision['quantityPrecision'])

    url = f"{BASE_URL}{REQUEST_PATH}"

    for i in range(ORDER_COUNT):
        logging.info(f"Placing order {i + 1} of {ORDER_COUNT} for {symbol_name}")
        timestamp = int(time.time() * 1000)
        ...
        try:
            ...
            data = response.json()
            logging.info(f"Order {i + 1} placed successfully for {symbol_name}: {data}")
        except requests.HTTPError as http_err:
            logging.error(f"HTTP error occurred while placing order {i + 1} for {symbol_name}: {http_err} - Response: {response.text}")
        except requests.RequestException as e:
            logging.error(f"Error placing order {i + 1} for {symbol_name}: {e}")

        time.sleep(WAIT_TIME)  # To avoid rate limits
...
if __name__ == '__main__':
    ...
    # Define the trade thread function
    def trade_thread_function():
        tradable_list = tradable_symbols  # Use already fetched tradable symbols
        if not tradable_list:
            logging.error("No tradable symbols available for trading.")
            return
        ...
        logging.info("Trade thread completed all trades.")

    # Start the periodic selling in a separate thread
    sell_thread = threading.Thread(target=sell_all_assets_periodically, args=(api_key, secret_key, asset_summary, LIQUIDATION_TIME), daemon=True)
    sell_thread.start()
    logging.info("Sell thread started.")

    # Start the trade thread
    thread = threading.Thread(target=trade_thread_function, daemon=True)
    thread.start()
    logging.info("Trade thread started.")

    # Keep the main thread alive until trade_thread completes
    thread.join()
    logging.info("All trades completed. Program exiting.")
