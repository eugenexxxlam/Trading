...
# Fetch precision, mappings for pairs, and tradable symbols
def fetch_pairs_precision():
    ...
    tradable_symbol_map = {s['baseAsset'].lower(): f"{s['baseAsset']}/{s['quoteAsset']}" for s in data if s['quoteAsset'] == 'USDT'}
    return precision_data, asset_to_symbol_map, tradable_symbol_map
...
# Modify balance_snapshot to include the tradable symbol
def balance_snapshot(api_key, secret_key):
    account_info = fetch_account_info(api_key, secret_key)
    if not account_info:
        return

    precision_data, asset_to_symbol_map, tradable_symbol_map = fetch_pairs_precision()
    ...
    for asset, balance in account_balances.items():
        ...
        asset_key = asset.lower().replace("1802", "")  # Remove suffix for mapping
        precision = precision_data.get(f"{asset_key}usdt", {'pricePrecision': 'N/A', 'quantityPrecision': 'N/A'})
        tradable_symbol = tradable_symbol_map.get(asset_key, "N/A")  # Fetch tradable symbol
        rows.append({
            "Asset": asset,
            "Symbol": f"{asset.lower()}usdt1802",
            "Tradable Symbol": tradable_symbol,
    ...
    # Sort rows by Value in descending order and update the PrettyTable field names
    rows.sort(key=lambda x: float(x["Value (USDT)"].replace(',', '')) if x["Value (USDT)"] != "N/A" else 0, reverse=True)
    table = PrettyTable(field_names=["Asset", "Symbol", "Tradable Symbol", "Free Balance", "Bid Price", "Value (USDT)", "Price Precision", "Quantity Precision"])
    for row in rows:
