...
def place_market_order(api_key, secret_key, contract_name, volume, side, open_position='OPEN', position_type=1, recv_window=5000):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"Market order placed successfully: {data}")
        else:
            print(f"Failed to place market order. Status Code: {response.status_code}, Response: {response.text}")
...
if __name__ == '__main__':
    ...
    # Loop to place smaller orders
    for i in range(PARTS):
        print(f"Placing order {i + 1} of {PARTS}...")
        place_market_order(api_key, secret_key, contract_name=CONTRACT_NAME, volume=VOLUME_PER_ORDER, side=SIDE, open_position=OPEN_POSITION, position_type=POSITION_TYPE, recv_window=RECV_WINDOW)
