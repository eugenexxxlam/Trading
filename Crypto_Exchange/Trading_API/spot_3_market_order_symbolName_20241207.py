...
SYMBOL_NAME = 'ORDI/USDT'  # Updated symbol name for the currency pair
VOLUME = 0.1 # Amount to buy/sell
...
def place_spot_market_order(api_key, secret_key, symbol_name, volume, side, order_type='MARKET', recv_window=5000):
    ...
    params = {
        'symbolName': symbol_name,  # Use symbolName instead of symbol
        'volume': volume,
        'side': side,
        'type': order_type,
        'timestamp': timestamp,
...
if __name__ == '__main__':
    ...
    for i in range(ORDER_COUNT):
        ...
        place_spot_market_order(api_key, secret_key, symbol_name=SYMBOL_NAME, volume=VOLUME, side=SIDE, order_type=ORDER_TYPE)
