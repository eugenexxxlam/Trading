...
if __name__ == '__main__':
    ...
    # Display assets above a certain threshold and ending with '1802'
    threshold = THRESHOLD
    ...
    if assets_above_threshold:
        ...
        for asset, balance in assets_above_threshold.items():
            symbol = f"{asset.lower()}usdt1802"
            base_asset = asset[:-4]  # Extract base asset
            bid_price = 1.0 if asset == "USDT1802" else fetch_best_bid(asset)
            value = balance * bid_price if bid_price else 0

            # Try to find precision using symbol or base asset
            precision = precision_data.get(symbol, asset_to_symbol_map.get(base_asset.lower(), {'pricePrecision': 'N/A', 'quantityPrecision': 'N/A'}))

            rows.append([asset, symbol, base_asset, balance, bid_price, value, precision['pricePrecision'], precision['quantityPrecision']])

        # Sort rows by Value (USDT) in descending order
        rows.sort(key=lambda x: x[5], reverse=True)

        # Create PrettyTable and add sorted rows
        table = PrettyTable()
        table.field_names = ["Asset", "Symbol", "Base Asset", "Free Balance", "Bid Price", "Value (USDT)", "Price Precision", "Quantity Precision"]

        for row in rows:
            asset, symbol, base_asset, balance, bid_price, value, price_precision, quantity_precision = row
            bid_display = f"{bid_price:,.8f}" if bid_price else "N/A"
            value_display = f"{value:,.8f}" if bid_price else "N/A"
            table.add_row([asset, symbol, base_asset, f"{balance:,.8f}", bid_display, value_display, price_precision, quantity_precision])

        print("\nAssets Above Threshold (Sorted by Value):")
        print(table)
    else:
